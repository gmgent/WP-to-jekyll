  ...
  channel.each do |channel_item|
    if channel_item[0]=="item"
      channel_item[1].each do |post_item|
        ...
        puts 
        puts "title: #{post_item["title"]}"
        #puts post_item["encoded"]
        
        
        linkname = post_item["title"].to_s.gsub!(" ", "-") ? post_item["title"].to_s.gsub!(" ", "-") : post_item["title"].to_s
        ...
        File.open(physical_file_name, 'w') do |f|
          ...
          f.puts "categories: [Song]\n"

          f.puts "---"
          f.puts
          f.puts post_item["encoded"][0].to_s
        end
